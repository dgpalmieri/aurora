...
#define LON_SHIFT  20.0

#if 0
#define MAX_CURTAINS 9
static char *filenames[MAX_CURTAINS] = {
  "curtainMain.data",
  "secondaryCurtain.data", "sec2.data", "sec3.data",
  "ter0.data", "tertiaryCurtain.data", "ter2.data", "ter3.data",
  "clouds.data" }; 
#endif
#if 1
  ...
  for ( c=0 ; c<MAX_CURTAINS ; c++ ) {
    ...
    while (fgets(inputline, 255, fp) != NULL) {
      ...
      if ((inputline[0] >= '0' && inputline[0] <= '9')  ||
        ...
        cart.x = X/mag; cart.y = Y/mag; cart.z = Z/mag;

        /* calculate the lat/long of this point */
        p = cartesian2polar(cart);
/*
        printf("<%.2lf,%.2lf,%.2lf> -> ", X, Y, Z);
        printf("la=%.2lf lo=%.2lf -> ", p.lat, p.lon);
*/

        /* adjust lat/long to center of interest */
        p.lat += LAT_SHIFT;
        p.lon += LON_SHIFT;
        if (p.lon < -180.0) p.lon += 360.0;
        else if (p.lon > 180.0) p.lon -= 360.0;

        /* convert point back to cartesian space */
        cart = polar2cartesian(p);
