...
Noise::Noise() {
    std::random_device seeding_device;
    std::mt19937 random_generator(seeding_device());

    for (int i = 0; i < NUMPTS; ++i)
        pts[i] = ((float)random_generator() / (float)random_generator.max());
}

Noise::Noise(int seeding) {
    std::mt19937 random_generator(seeding);
